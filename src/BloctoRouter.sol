...
contract Router {
    ...
    address public immutable BLOCTO_GATE;
    ...
    constructor(address _WETH, address _owner, address _factory, address _sDYSON, address _DYSON, address _USDC, address _BLOCTO_GATE) {
    ...
    function deposit(address tokenIn, address tokenOut, uint index, address to, uint input, uint minOutput, uint time) external returns (uint output) {
        BloctoGateToken gateToken = BloctoGateToken(BLOCTO_GATE);
        tokenIn.safeTransferFrom(msg.sender, address(this), input);
        output = _deposit(tokenIn, tokenOut, index, to, input, minOutput, time);
        
        uint balance = tokenIn == USDC ? input : output;
        if(balance > 20e6 && gateToken.balanceOf(msg.sender) == 0) {
            gateToken.mint(1);
        }
    }
    ...
    function depositETH(address tokenOut, uint index, address to, uint minOutput, uint time) external payable returns (uint output) {
        IWETH(WETH).deposit{value: msg.value}();
        output = _deposit(WETH, tokenOut, index, to, msg.value, minOutput, time);
        
        // MARK: need to verify
        if(output > 20e6 && BloctoGateToken(BLOCTO_GATE).balanceOf(msg.sender) == 0) {
            BloctoGateToken(BLOCTO_GATE).mint(1);
        }
    }
